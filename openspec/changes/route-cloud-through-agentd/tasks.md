...
- [x] 4.5 Обновить раздел «Подключение» (`Ctrl+P`) в TUI: адрес сервиса, токен
  и модель для облачного чата, раздел Ollama без изменений, сохранение по
  `Ctrl+S` применяется к следующим запросам без перезапуска. Проверка: сборка
  и запуск клиента; интерактивный прогон TUI пользователем не выполнялся.

## 5. Перевод сервиса на `agentupstream`

- [x] 5.1 Запушить `agent-cli` в `main` (разделение крейтов и правки ядра).
...
- [x] 5.3 Заменить в сервисе `agentcore::agent::HttpAgent` на агента из
  ...
  локального `[patch]`.
- [x] 5.4 Проверить сборку против запушенного `main`: временно убрать
...
- [x] 6.1 Обновить README `agent-cli`: новая схема запуска (сначала `agentd`,
  ...
  workspace.
- [x] 6.2 Обновить README `agent-sever`: сервис как единственный владелец ключа
  провайдера, зависимость на `agentupstream`, локальный `[patch]` на два
  крейта.
- [x] 6.3 Итоговая проверка: `cargo build` и `cargo test` зелёные в обоих
